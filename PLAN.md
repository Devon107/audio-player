...
- [ ] Generar y probar builds para Windows (.msi/.exe) y macOS (.dmg) — **no es posible desde este entorno**: son builds nativos que requieren compilar (y, para macOS, firmar/notarizar) en esa plataforma real; Tauri no soporta cross-compilar el empaquetado completo desde Linux. El `tauri.conf.json` ya está configurado para generarlos (íconos `.ico`/`.icns` listos, metadata de bundle completa) — falta correr `tauri build` en una máquina Windows/macOS real la primera vez que haya acceso a una
