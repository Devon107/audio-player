...
- [x] Bug reportado al probar con una carpeta de +2000 pistas: el conteo del pie de página se quedaba fijo en "1000 tracks" sin reflejar el total real. Causa: `tracks.length` en el frontend solo veía las filas traídas por `list_tracks` (topeadas en 1000), no el total de la biblioteca. Se agregó `count_tracks` (query + comando, mismo filtro que `list_tracks` pero `SELECT COUNT(*)` sin `LIMIT`), y el pie de página ahora distingue "N canciones" (cuando entran todas) de "Mostrando 1000 de N" (cuando el tope las corta)
- [x] Paginación real implementada (scroll infinito): `loadMoreTracks` en `libraryStore` pide la siguiente tanda de `TRACKS_PAGE_SIZE` (500) pistas cuando el scroll de la tabla se acerca al final (`LibraryView`'s `onScroll`, umbral de 300px), y las agrega a la lista ya cargada en vez de reemplazarla — con un indicador de carga chico al pie mientras trae la página siguiente. Se protegió contra una condición de carrera: si el filtro cambia mientras una página está en vuelo, la respuesta vieja se descarta en vez de mezclarse con resultados de un filtro distinto
- [x] Bug reportado al probar el ancho fijo de los `<select>` de artista/álbum/género: la caja cerrada ya no se estiraba con nombres largos, pero el popup de opciones desplegado sí — nombres de ~15-20 palabras hacían que la lista de opciones se saliera de la pantalla. `text-overflow`/`truncate` de CSS no afecta a `<option>` nativos dentro del popup abierto (limitación del control nativo, no del CSS) — se agregó `truncateText` (`lib/format.ts`) para cortar el string a mano a 40 caracteres antes de renderizarlo como texto de la opción
- [x] Bug reportado por el usuario: al quitar una carpeta vigilada, los filtros de artista/álbum/género seguían mostrando datos de esa carpeta aunque la tabla de canciones sí había quedado limpia. Causa: `list_artists`/`list_albums`/`list_genres` usaban `LEFT JOIN` contra `tracks`, así que una fila de `artists`/`albums`/`genres` sin ninguna pista restante (huérfana tras el borrado) seguía apareciendo con `track_count: 0` en vez de desaparecer del filtro; y ningún camino de borrado (`remove_tracks_under`, rescan detectando archivos borrados, el watcher en vivo) limpiaba esas tablas, solo `tracks`. Se corrigió en dos niveles: las tres consultas pasaron a `INNER JOIN` (nunca muestran huérfanos, se auto-corrige aunque la base ya tenga inconsistencias); y se agregó `library::scanner::cleanup_orphaned_taxonomy`, que borra artistas/álbumes/géneros sin pistas y se llama tras cada borrado de pistas (`remove_missing_tracks`, `remove_track_by_path`) y una vez al arrancar la app para barrer huérfanos de datos ya existentes en la base del usuario. Efecto secundario relacionado detectado en la misma prueba: la cola de reproducción y la barra inferior seguían mostrando (y hasta reproduciendo) canciones de la carpeta recién quitada, porque `QueueState` en el motor de audio es un subsistema aparte de la base de datos — quitar filas de `tracks` no le avisa nada. Se agregó `QueueState::remove_under` (saca de la cola cualquier pista cuyo archivo esté bajo la carpeta quitada, y si la pista actual era una de ellas, corta la reproducción en vez de dejarla terminar como hace `remove()` con una pista individual) y el comando `AudioCommand::RemoveQueueItemsUnder`, disparado desde `remove_watched_folder` junto con la limpieza de la base de datos
...
- [x] Modo aleatorio: `set_shuffle` mezcla físicamente `items` (dejando la pista actual primero), y se vuelve a mezclar al llegar al final si `repeat: queue` también está activo — así la cola que ve el usuario en el panel siempre refleja el orden real en que se va a reproducir (rediseñado durante la revisión manual; el diseño original usaba una "bolsa" de orden interno separada de `items`, ver Fase 8)
...
- [x] Botones de control en UI (`PlayerBar`): anterior, siguiente, play/pause, shuffle, repeat (cíclico off→queue→track), barra de progreso con seek (arrastre), volumen — el volumen usa una curva cuadrática (posición del slider²) al convertir a la ganancia lineal real, porque el oído percibe el volumen logarítmicamente y una ganancia lineal se sentía "sin cambios" hasta cerca del máximo. Se probó primero con una curva cúbica, pero quedó demasiado agresiva (a 50% del slider sonaba notablemente más bajo que otras apps de referencia, p. ej. YouTube Music, en la misma posición); se pasó a cuadrática y quedó parejo
- [x] Panel de cola (`QueuePanel`) con reordenamiento por drag-and-drop nativo (HTML5), quitar ítems y saltar a cualquier pista
- [x] Sincronización en tiempo real vía eventos Tauri: `player://loaded`, `player://progress` (incluye `is_playing`, para reflejar cambios de play/pause disparados desde afuera de la UI — teclas multimedia, Bluetooth), `player://track-ended`, `player://queue-changed` (snapshot completo: ítems, pista actual, shuffle, repeat, `has_previous`, duración y posición de la pista actual), `player://error`
- [x] Persistencia de reproducción entre sesiones: volumen (`settings::commands::get/set_volume_preference`, mismo patrón que idioma/EQ) y cola completa + pista actual + posición (`audio::output::persist_queue`/`load_persisted_queue`, guardados en `settings` como JSON bajo la clave `queue_state`). Al arrancar, el motor restaura la cola y carga la pista actual **en pausa** (no arranca a reproducir sola) con un seek a la posición guardada — así la barra de reproducción muestra el último track escuchado, listo para continuar. La posición se guarda cada 5s mientras suena (no en cada tick de progreso, para no escribir a SQLite 4 veces por segundo) más al pausar/buscar; la cola en sí se guarda en cada cambio estructural (agregar/quitar/reordenar/cambiar de pista/shuffle/repeat)
- [x] Bug reportado al probar lo de arriba: con aleatorio activo, cerrar y reabrir la app no restauraba la pista/cola exactas — cambiaba la pista actual y reordenaba la cola de nuevo, como si hubiera vuelto a mezclar. Causa: el restore llamaba a `queue.set_shuffle(persisted.shuffle)` _antes_ de fijar la pista actual (`play_item`) — y `set_shuffle(true)` no solo prende la bandera, también dispara `shuffle_keeping_current_first()` (efecto colateral pensado para cuando el usuario activa aleatorio a mano). Sin una pista actual todavía establecida en ese momento, mezclaba la lista entera sin ancla, y el `current_index` guardado (que apuntaba a la lista original) terminaba señalando una pista distinta en la lista ya remezclada. Se agregó `QueueState::set_shuffle_flag`, que solo prende la bandera sin el efecto colateral de mezclar — usado exclusivamente en el restore, así aleatorio solo dispara una mezcla nueva cuando el usuario lo activa desde la UI, nunca al restaurar un estado ya guardado. Verificado con una prueba de humo manual nueva (`restoring_a_shuffled_queue_does_not_reshuffle_it`) y en vivo
...
- [x] Auditoría de permisos: se encontró que `tauri-plugin-opener` estaba registrado (Rust, JS y en `capabilities/default.json`) pero nunca se usaba desde el frontend — se quitó por completo (dependencia, plugin, permiso), dejando `capabilities/default.json` con solo `core:default` y `dialog:default`, que es todo lo que la app realmente necesita: el resto del acceso a archivos (escanear carpetas, leer audio, extraer carátulas) lo hace el backend Rust directamente con `std::fs`/`walkdir`, no a través de un plugin con permisos propios, así que no necesita entradas de capability adicionales
- [x] Metadata de bundle completada en `tauri.conf.json`: `category: "Music"`, `shortDescription`/`longDescription`, `homepage` (repo de GitHub). **Pendiente**: `Maintainer`/`publisher` quedó en el placeholder "you" (heredado de `authors` en `Cargo.toml`, nunca completado) — hay que poner un nombre/email real antes de distribuir el paquete de verdad
- [x] Bug encontrado al probar el paquete instalado: pantalla en blanco con "Could not connect to localhost: Connection refused". Dos causas distintas, corregidas las dos: (1) el `.desktop` local que se había instalado a mano (nota de arriba) apuntaba al **binario de debug** (`target/debug`), que solo funciona si `bun run tauri dev` está corriendo con el servidor de Vite activo — sin eso intenta conectarse a `http://localhost:1420` y falla; se corrigió apuntándolo al `.AppImage` autocontenido en vez del binario de debug. (2) Bug real y ya documentado de Tauri v2: `vite build` generaba `dist/index.html` con rutas absolutas (`/assets/...`), pero el webview de producción de Tauri no sirve los archivos como un servidor con raíz `/` — los intercepta vía su propio protocolo interno — así que con rutas absolutas el build empaquetado (`.deb`/`.AppImage`, y también hubiera afectado a Windows/macOS) queda con pantalla en blanco de verdad, no solo en el `.desktop` de prueba. Se agregó `base: './'` en `vite.config.ts` para que Vite emita rutas relativas, y se regeneraron los tres paquetes de Linux con el fix
- [x] Build real generado y verificado en Linux (`bun run tauri build`): `.deb` (7.5M), `.rpm` (7.5M) y `.AppImage` (83M, el AppImage es más grande porque empaqueta sus propias libs de WebKitGTK). Se probó el `.AppImage` end-to-end: arranca, monta por FUSE, registra el servicio MPRIS (`org.mpris.MediaPlayer2.audio_player`) y lee la misma `library.sqlite` que ya venía usando en modo desarrollo — confirma que mantener el `identifier` sin cambios (ver nota de la Fase 8) no rompió nada. También se verificó el contenido del `.deb`: instala su propio `Vinilo.desktop` + íconos en `/usr/share/icons/hicolor/*/apps/`, así que en una instalación real (`dpkg -i`) el ícono del dock funciona solo, sin necesitar el `.desktop` manual que se instaló a mano para probar en modo `dev` (ver Fase 8)
- [x] Instalación real verificada: `sudo dpkg -i Vinilo_0.1.0_amd64.deb` en el propio equipo de desarrollo. Abre desde el selector de aplicaciones sin pantalla en blanco, escanea la biblioteca y reproduce audio con normalidad — el `.desktop` manual que se había instalado a mano (nota de más arriba) ya no hace falta y se eliminó, la instalación real de `dpkg` genera el suyo propio. Nota aparte, no relacionada a un bug de la app: durante la prueba el audio no se escuchaba a través de un parlante Bluetooth (JBL Go 3) — resultó ser una rareza de PipeWire/BlueZ (un stream nuevo que arranca mientras el transporte A2DP ya está en uso por otra app a veces queda "conectado" sin mezclarse realmente hasta que algo fuerza una renegociación del grafo de audio); no es nada para corregir en el código de Vinilo
- [x] Workflow de GitHub Actions (`.github/workflows/build.yml`) para compilar Windows (.msi), macOS (.dmg, arm64 + x86_64 por separado) y Linux (.deb/.rpm/.AppImage) sin necesitar máquinas propias — usa runners de GitHub (`windows-latest`/`macos-latest`/`ubuntu-22.04`) vía `tauri-apps/tauri-action`, con `libasound2-dev` agregado a las dependencias de Ubuntu (específico de este proyecto, para que compile el motor de audio). Se dispara manualmente o en cada push a `master`; no crea una GitHub Release, solo deja los instaladores como artifacts descargables — pensado para "compilar y probar", no para publicar todavía. **Pendiente**: aunque ahora se puede _compilar_ Windows/macOS sin hardware propio, siguen sin poder _probarse_ en uso real (instalar y usar la app) sin acceso a esas plataformas — CI solo confirma que compila y empaqueta, no que funcione en la práctica (controles de medios nativos, etc.)
- [x] Primer build de macOS probado en hardware real (arm64/Apple Silicon, compilado vía el workflow de GitHub Actions): la app abre y el audio se escucha correctamente. Tuvo que sortearse el bloqueo de Gatekeeper (mensaje "la app está dañada, moverla a la papelera") con `xattr -cr Vinilo.app` desde Terminal — es el rechazo estándar de macOS a cualquier build sin firmar descargado de internet, no un bug del build en sí; sin firma/notarización de Apple Developer esto le va a pasar a cualquiera que pruebe el `.dmg` de los artifacts de CI. Nota aparte: el usuario encontró una app preexistente en su Mac también llamada "Vinilo" (de lectura de documentos de texto, no relacionada) — coincidencia de nombre, no causó el problema de Gatekeeper, pero es señal de que el nombre podría no ser único si se llega a publicar en la Mac App Store algún día
- [x] Verificar permisos de acceso a sistema de archivos en cada plataforma — confirmado en las tres (Linux, macOS, Windows)
- [x] Probar rendimiento de audio (latencia, glitches) en cada SO — confirmado en las tres; en Windows además se notó que el escaneo de la biblioteca corre notablemente más rápido que en Linux (dato observado, no algo para "corregir")
- [x] Controles nativos de medios probados en macOS (MacBook Pro con Touch Bar) y en Windows: las teclas de medios funcionan en ambos, controlando play/pause/siguiente/anterior a través de `souvlaki` (MPRIS en Linux, Now Playing en macOS, SMTC en Windows) — confirma que la integración nativa de medios (Fase 8) funciona en las tres plataformas. También se confirmaron los atajos de teclado propios de la app (espacio, flechas) en las tres
- [x] Instalación probada en Windows (build de CI, sin firmar): apareció la advertencia de SmartScreen ("Windows protegió su PC"), hubo que elegir "Más información" → "Ejecutar de todas formas" — mismo tipo de bloqueo que Gatekeeper en macOS, mismo motivo (build sin certificado de firma de código), no un bug del build. Una vez aceptado, abre, escanea y reproduce con normalidad
- [ ] Firmar/notarizar build de macOS (requiere Mac + cuenta de Apple Developer, $99/año) y firmar el build de Windows (requiere un certificado de firma de código, con costo variable según el proveedor) si se distribuyen fuera de tiendas de apps — opcional, no evaluable acá. Mientras tanto, cualquiera que pruebe los builds necesita los workarounds ya documentados (`xattr -cr` en macOS, "Ejecutar de todas formas" en Windows)

Nota sobre el `identifier`: `tauri build` avisa que `"com.audioplayer.app"` termina en `.app`, lo cual choca con la extensión de bundle de macOS (`.app`) y no es recomendado — es el precio de la decisión tomada en la Fase 8 de no tocar el `identifier` para no reubicar `app_data_dir`/`app_cache_dir` y perder la biblioteca ya escaneada. Antes de generar el build de macOS habría que decidir si vale la pena cambiarlo (con su correspondiente migración de datos) o convivir con el warning, que por ahora no impide compilar ni empaquetar.
