...
- [x] Workflow de GitHub Actions (`.github/workflows/build.yml`) para compilar Windows (.msi), macOS (.dmg, arm64 + x86_64 por separado) y Linux (.deb/.rpm/.AppImage) sin necesitar máquinas propias — usa runners de GitHub (`windows-latest`/`macos-latest`/`ubuntu-22.04`) vía `tauri-apps/tauri-action`, con `libasound2-dev` agregado a las dependencias de Ubuntu (específico de este proyecto, para que compile el motor de audio). Se dispara manualmente o en cada push a `master`; no crea una GitHub Release, solo deja los instaladores como artifacts descargables — pensado para "compilar y probar", no para publicar todavía. **Pendiente**: aunque ahora se puede *compilar* Windows/macOS sin hardware propio, siguen sin poder *probarse* en uso real (instalar y usar la app) sin acceso a esas plataformas — CI solo confirma que compila y empaqueta, no que funcione en la práctica (controles de medios nativos, etc.)
- [ ] Verificar permisos de acceso a sistema de archivos en cada plataforma — hecho en Linux (ver arriba); en Windows/macOS también depende de tener esas plataformas disponibles para probar (macOS en particular puede pedir permisos de acceso a carpetas la primera vez, algo que no se puede simular sin el SO real)
- [ ] Probar rendimiento de audio (latencia, glitches) en cada SO — hecho en Linux durante toda la sesión; Windows/macOS quedan pendientes por el mismo motivo
- [ ] Firmar/notarizar build de macOS si se distribuye fuera de la App Store (opcional) — requiere una Mac + cuenta de Apple Developer, no evaluable acá
