...
- [ ] Verificar permisos de acceso a sistema de archivos en cada plataforma — hecho en Linux y macOS (ver arriba); Windows queda pendiente por no tener esa plataforma disponible para probar
- [ ] Probar rendimiento de audio (latencia, glitches) en cada SO — hecho en Linux y macOS (audio confirmado); Windows queda pendiente
- [x] Controles nativos de medios probados en macOS (MacBook Pro con Touch Bar): las teclas de medios funcionan, controlando play/pause/siguiente/anterior a través de `souvlaki` igual que en Linux (MPRIS) — confirma que la integración nativa de medios (Fase 8) también funciona en macOS, no solo en Linux. También se confirmaron los atajos de teclado propios de la app (espacio, flechas) andando en macOS. Windows sigue sin probarse (controles nativos vía SMTC, sin acceso a esa plataforma)
- [ ] Firmar/notarizar build de macOS si se distribuye fuera de la App Store (opcional) — requiere una Mac + cuenta de Apple Developer ($99/año), no evaluable acá. Mientras tanto, cualquiera que pruebe el build necesita el workaround de `xattr -cr` documentado arriba
